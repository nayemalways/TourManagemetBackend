...
passport.use(
    new LocalStrategy({
        ...
        try {
            ...
            const isGoogleAuthenticated = isUserExists.auths?.some(provider => provider.provider === "google");
 

            if(isGoogleAuthenticated && !isUserExists.password) {
                return done(null, false, {message: "You are authenticated through Google. So if you want to login with credentials, then at first login with google and set a password to your gmail and then you can login with email and password"})
            }
