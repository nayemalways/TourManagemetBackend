...
const failedPayment = CatchAsync(async (req: Request, res: Response, next: NextFunction) => {
    const query = req.query;
    const result = await paymentServices.paymentFailService(query as Record<string, string>);
    
    if (!result.success) {
        res.redirect(`${env.CLIENT_FAIL_URL}/transaction_id=${query.transaction_id}&amount=${query.amount}&status=${query.status}`);
    }
})
const cancelPayment = CatchAsync(async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!result.success) {
        res.redirect(`${env.CLIENT_CANCEL_URL}/transaction_id=${query.transaction_id}&amount=${query.amount}&status=${query.status}`);
    }
})
